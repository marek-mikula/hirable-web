...
import type {TokenInvitation, Company} from "~/repositories/resources";
...
async function getInvitations(query: GridQueryString) {
  return (await api.companyInvitation.index(props.company.id, query))._data!.data.invitations
}
...
async function onDelete(invitation: TokenInvitation): Promise<void> {
  ...
  const result = await handle(async () => api.companyInvitation.deleteInvitation(props.company.id, invitation.id))
