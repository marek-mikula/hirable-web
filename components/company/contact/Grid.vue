...
async function getContacts(query: GridQueryString) {
  return (await api.companyContact.index(props.company.id, query))._data!.data.contacts
}
...
async function onDelete(contact: CompanyContact): Promise<void> {
  ...
  const result = await handle(async () => api.companyContact.deleteContact(props.company.id, contact.id), async (e) => {
